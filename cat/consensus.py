...
def generate_consensus(args):
    ...
    coding_df, non_coding_df = combine_and_filter_dfs(
        tx_dict,
        hgm_df,
        mrna_metrics_df,
        cds_metrics_df,
        ...
        ref_df,
        eval_df,
        args.intron_rnaseq_support,
        args.exon_rnaseq_support,
        args.intron_annot_support,
        args.exon_annot_support,
        args.original_intron_support,
        args.in_species_rna_support_only,
    )
    if len(coding_df) + len(non_coding_df) == 0:
        raise RuntimeError(
            "No transcripts pass filtering for species {}. "
            "Consider lowering requirements. Please see the manual.".format(args.genome)
        )
    elif len(coding_df) == 0 and coding_count > 0:
        logger.warning(
            "No protein coding transcripts pass filtering for species {}. "
            "Consider lowering requirements. Please see the manual.".format(args.genome)
        )
    elif len(non_coding_df) == 0 and non_coding_count > 0:
        logger.warning(
            "No non-coding transcripts pass filtering for species {}. "
            "Consider lowering requirements. Please see the manual.".format(args.genome)
        )
    scored_coding_df, scored_non_coding_df = score_filtered_dfs(
        coding_df, non_coding_df, args.in_species_rna_support_only
    )
    scored_df = merge_scored_dfs(scored_coding_df, scored_non_coding_df)
    best_alignments = (
        scored_df.groupby("TranscriptId")["TranscriptScore"].transform(max) == scored_df["TranscriptScore"]
    )
    ...
    if len(args.denovo_tx_modes) > 0:
        ...
        denovo_dict = find_novel(
            ...
            args.denovo_splice_support,
            args.denovo_exon_support,
            args.denovo_ignore_novel_genes,
            args.denovo_novel_end_distance,
            args.denovo_allow_unsupported,
            args.denovo_allow_bad_annot_or_tm,
            args.denovo_only_novel_genes,
            args.denovo_allow_novel_ends,
        )
    ...
    if "augPB" in args.denovo_tx_modes:
        deduplicated_strand_resolved_consensus = validate_pacbio_splices(
            deduplicated_strand_resolved_consensus, args.db_path, tx_dict, metrics, args.require_pacbio_support
        )

    if args.filter_overlapping_genes is True:
        gene_resolved_consensus = resolve_overlapping_cds_intervals(
            args.overlapping_gene_distance, deduplicated_strand_resolved_consensus, tx_dict
        )
    else:
    ...
    final_consensus = sorted(
        gene_resolved_consensus, key=lambda tx_attrs: (tx_dict[tx_attrs[0]].chromosome, tx_dict[tx_attrs[0]].start)
    )
    ...
    consensus_gene_dict = write_consensus_gps(
        args.consensus_gp, args.consensus_gp_info, final_consensus, tx_dict, args.genome
    )
...
def calculate_vector_support(s, resolve_nan=None, num_digits=4):
    ...
    return 100 * tools.mathOps.format_ratio(
        len([x for x in s if x > 0]), len(s), resolve_nan=resolve_nan, num_digits=num_digits
    )
...
def load_metrics_from_db(db_path, tx_mode, aln_mode):
    ...
    metrics_df["OriginalIntrons"] = [
        list(map(int, x)) if len(x[0]) > 0 else [] for x in metrics_df["OriginalIntrons"].str.split(",").tolist()
    ]
...
def load_evaluations_from_db(db_path, tx_mode):
    ...
    merged = mrna_df.reset_index().merge(
        cds_df.reset_index(), how="outer", on=["AlignmentId", "name"], suffixes=["_mRNA", "_CDS"]
    )
...
def combine_and_filter_dfs(
    ...
    exon_rnaseq_support,
    intron_annot_support,
    exon_annot_support,
    original_intron_support,
...
def score_filtered_dfs(coding_df, non_coding_df, in_species_rna_support_only):
    ...
    def score(s):
        ...
        orig_intron = (
            s.OriginalIntronsPercent_mRNA
            if s.TranscriptBiotype == "protein_coding"
            else s.TransMapOriginalIntronsPercent
        )
...
def incorporate_tx(best_rows, gene_id, metrics, hints_db_has_rnaseq):
    ...
    d = {
        "source_transcript": best_series.TranscriptId,
...
def find_novel(
    ...
    in_species_rna_support_only,
    denovo_tx_modes,
    denovo_splice_support,
    denovo_exon_support,
    denovo_ignore_novel_genes,
    denovo_novel_end_distance,
    denovo_allow_unsupported,
    denovo_allow_bad_annot_or_tm,
    denovo_only_novel_genes,
    denovo_allow_novel_ends,
):
    """
    ...
    def is_novel_supported(s):
        ...
        if len(denovo_tx_obj.intron_intervals) < denovo_num_introns:
            ...
            in_species_rna_support_only
            and s.ExonRnaSupportPercent <= denovo_exon_support
            or s.IntronRnaSupportPercent <= denovo_splice_support
        ):
            return None
        elif (
            in_species_rna_support_only is False
            and s.AllSpeciesExonRnaSupportPercent <= denovo_exon_support
            or s.AllSpeciesIntronRnaSupportPercent <= denovo_splice_support
        ):
            return None
        ...
        if any(
            annot == 0 and i in new_supported_splices
    ...
    def has_novel_ends(s):
        ...
        five_p_matches = tools.intervals.interval_not_within_wiggle_room_intervals(
            existing_5p[denovo_tx_obj.chromosome], five_p, denovo_novel_end_distance
        )
        three_p_matches = tools.intervals.interval_not_within_wiggle_room_intervals(
            existing_5p[denovo_tx_obj.chromosome], three_p, denovo_novel_end_distance
        )
        if denovo_allow_novel_ends is False:
            ...
            tx_class = (
                ...
                if s.TranscriptClass is None and (five_p_matches or three_p_matches)
                else s.TranscriptClass
            )
    ...
    if denovo_ignore_novel_genes is True:
        filtered_denovo_df = filtered_denovo_df[
            (filtered_denovo_df.TranscriptClass == "possible_paralog")
    ...
    for _, s in filtered_denovo_df.iterrows():
        ...
        denovo_tx_dict[aln_id] = {
            "source_gene": s.AssignedGeneId,
    ...
    for tx_mode, df in denovo_df.groupby("TranscriptMode"):
        metrics["denovo"][tx_mode]["Discarded"] = len(
            df[(~df.TranscriptClass.isnull()) | (~df.Novel5pCap.isnull()) | (~df.NovelPolyA.isnull())]
        )
...
def deduplicate_consensus(consensus_dict, tx_dict, metrics):
    ...
    def resolve_duplicate(tx_list, consensus_dict):
        biotype_txs = [
            tx
            for tx in tx_list
            if consensus_dict[tx].get("gene_biotype", None) == consensus_dict[tx].get("transcript_biotype", None)
        ]
        if len(biotype_txs) > 0:
            tx_list = biotype_txs
        sorted_scores = sorted(
            [[tx, consensus_dict[tx].get("score", 0)] for tx in tx_list], key=lambda tx_s1: -tx_s1[1]
        )
        ...
        with open(tmp_gp, "w") as outf:
            for tx_id, attrs in deduplicated_strand_resolved_consensus:
                ...
                attr_df.append(
                    [
                        tx_id,
                        attrs["transcript_class"],
                        attrs["gene_biotype"],
                        attrs.get("source_gene", tx_obj.name2),
                        attrs.get("score", None),
                    ]
                )
        # cluster
        cmd = ["clusterGenes", "-cds", f"-minOverlappingBases={overlapping_gene_distance}", tmp_clustered, "no", tmp_gp]
...
def calculate_improvement_metrics(final_consensus, scored_df, tm_eval_df, hgm_df, metrics):
    ...
    for aln_id, c in final_consensus:
        if c["transcript_biotype"] != "protein_coding":
            ...
            continue
        tx_s = df.loc[aln_id]
        metrics["Evaluation Improvement"]["changes"].append(
            [
                tx_s.TransMapOriginalIntronsPercent,
...
def write_consensus_gff3(consensus_gene_dict, consensus_gff3):
    ...
    def generate_gene_record(chrom, tx_objs, gene_id, attrs_list):
        ...
        useful_keys = [
            ...
            "source_gene",
            "gene_biotype",
            "alternative_source_transcripts",
            "gene_alternate_contigs",
            "gene_name",
            "gene_id",
        ]
        ...
        for i, (exon, exon_frame) in enumerate(zip(*[tx_obj.exon_intervals, tx_obj.exon_frames])):
            ...
            if cds_interval is not None:
                ...
                yield [
                    ...
                    "CAT",
                    "CDS",
                    cds_interval.start + 1,
                    cds_interval.stop,
                    score,
                    exon.strand,
                    tools.transcripts.convert_frame(exon_frame),
                    attrs_field,
                ]
