...
def filter_transmap(
    ...
    with tools.fileOps.TemporaryFilePath() as local_tmp, tools.fileOps.TemporaryFilePath() as strip_tmp:
        ...
        cmd = [
            "pslCDnaFilter",
            "-globalNearBest={}".format(global_near_best),
            "-minCover=0.1",
            "-verbose=0",
            "-minSpan=0.2",
            strip_tmp,
    ...
    metrics = {
        "Paralogy": collections.defaultdict(lambda: collections.Counter()),
    ...
    coding_genes = {gene_id for gene_id, tx_list in global_best_by_gene.items() if any(x.cds_size > 0 for x in tx_list)}

    with tools.fileOps.TemporaryFilePath() as coding_tmp, tools.fileOps.TemporaryFilePath() as noncoding_tmp, tools.fileOps.TemporaryFilePath() as coding_clusters, tools.fileOps.TemporaryFilePath() as noncoding_clusters:
        ...
        cmd = [
            "clusterGenes",
            "-cds",
            f"-minOverlappingBases={overlapping_gene_distance}",
            coding_tmp,
            "no",
            coding_clusters,
        ]
    ...
    coding_merged_df, coding_collapse_filtered = filter_clusters(
        coding_clustered,
        transcript_gene_map,
        gene_name_map,
        scores,
    ...
    noncoding_merged_df, noncoding_collapse_filtered = filter_clusters(
        noncoding_clustered,
        transcript_gene_map,
        gene_name_map,
        scores,
    ...
    combined_tx_df["TranscriptId"] = [
        tools.nameConversions.strip_alignment_numbers(x) for x in combined_tx_df.AlignmentId
    ]
    ...
    resolved_df, split_gene_metrics = resolve_split_genes(
        tmp_size_filtered, transcript_gene_map, resolved_df, unfiltered_tx_dict
    )
...
def filter_clusters(
    clustered, transcript_gene_map, gene_name_map, scores, metrics, gene_biotype_map, filter_overlapping_genes
):
    """
    ...
    clustered["gene_id"] = [
        transcript_gene_map[tools.nameConversions.strip_alignment_numbers(x)] for x in clustered.gene
    ]
...
def find_split_genes(gene_id, g, resolved_interval, split_gene_data):
    ...
    intervals = collections.defaultdict(list)
    for aln in g:
        ref_i = tools.intervals.ChromosomeInterval(
            tools.nameConversions.strip_alignment_numbers(aln.q_name), aln.q_start, aln.q_end, "."
        )
...
def resolve_split_genes(tmp_size_filtered, transcript_gene_map, resolved_df, unfiltered_tx_dict):
    ...
    tx_intervals = {
        tx_id: unfiltered_tx_dict[aln_id].interval
        for tx_id, aln_id in zip(resolved_df.TranscriptId, resolved_df.AlignmentId)
    }
    ...
    split_gene_metrics = {
        "Number of contig split genes": len(split_gene_data["contig"]),
