...
def get_output_file(ext):
    ...
    f = osp.join(get_output_dir(), get_test_id() + ext)
    return f
...
def diff_files(exp_file, out_file):
    ...
    for l in diff:
        print(l, end=' ')
    assert len(diff) == 0, f"{exp_file} and {out_file} differ"

def diff_expected(ext):
    """diff expected and output files, with names computed from test id."""
    diffFiles(get_expected_file(ext), get_output_file(ext))
