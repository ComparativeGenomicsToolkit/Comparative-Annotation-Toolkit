...
def tm_metrics_plot(tm_metrics, ordered_genomes, biotypes, transcript_biotype_map, tm_coverage_tgt, tm_identity_tgt,
    ...
    for mode, tgt in tm_iter:
        ...
        cov_ident_badness_plot(biotypes, ordered_genomes, mode, tgt, df)
...
def consensus_metrics_plot(consensus_data, ordered_genomes, biotypes, coverage_tgt, identity_tgt, badness_tgt,
    ...
    for mode, tgt in cons_iter:
        ...
        df[mode] = 100 * df[mode]
        cov_ident_badness_plot(biotypes, ordered_genomes, mode, tgt, df, x=mode, y='genome')
...
def cov_ident_badness_plot(biotypes, ordered_genomes, mode, tgt, df, x=None, y=None):
    ...
    with tgt.open('w') as outf, PdfPages(outf) as pdf:
        ...
        generate_boxplot_violin_pair(df, ordered_genomes, title, xlabel, x=x, y=y)
        multipage_close(pdf)
        for biotype in biotypes:
            biotype_df = biotype_filter(df, biotype)
            if biotype_df is not None:
                title = '{} for biotype {}'.format(mode, biotype)
                generate_boxplot_violin_pair(biotype_df, ordered_genomes, title, xlabel, x=x, y=y)
                multipage_close(pdf)
...
def generate_boxplot_violin_pair(data, ordered_genomes, title, xlabel, hue=None, x=None, y=None):
