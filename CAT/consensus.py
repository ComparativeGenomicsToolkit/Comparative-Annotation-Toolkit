...
def consensus(args):
    ...
    # load all databases into two DataFrames, one for each alignment mode. Score the alignments
    aln_mode_dfs = {}
    for aln_mode in ['CDS', 'mRNA']:
        tgt_df = tools.sqlInterface.load_classifications(args.db_path, aln_mode, args.transcript_modes)
        intron_df = tools.sqlInterface.load_intron_vector(args.db_path, args.transcript_modes)
    ...
    # metrics will hold stats calculated during the process and passed back for plotting.
    metrics = {}
    if args.resolve_split_genes is True:
        tx_to_remove = resolve_split_genes(aln_eval_df, ref_df, tx_dict)
...
def merge_ref_tgt(ref_df, tgt_df, intron_df):
    ...
    :param ref_df: DataFrame from load_reference()
    :param tgt_df: DataFrame from load_classifications()
    :return: DataFrame
    """
...
def score_df(df):
    ...
    for (gene_name, tx_id, aln_id), s in df.iterrows():
...
def consensus_score(s):
    ...
    :return: float
    """
    score = 50 * (1 - s.Badness) + 25 * structure_score(s) + supported_junctions(s)
...
def structure_score(s):
    ...
    """
    present_introns = 1 - tools.mathOps.format_ratio(s.NumMissingIntrons, s.NumReferenceIntrons)
...
def supported_junctions(s):
    ...
    :return: float between 0 and 1
    """
    percent_supported = tools.mathOps.format_ratio(s.NumSupportedIntrons, s.NumIntrons)
    assert 0 <= percent_supported <= 1
