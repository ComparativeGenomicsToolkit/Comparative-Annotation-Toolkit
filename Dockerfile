...
RUN curl -LO http://hgdownload.soe.ucsc.edu/admin/exe/linux.x86_64/{axtChain,bamToPsl,bedSort,bedToBigBed,chainMergeSort,clusterGenes,faToTwoBit,genePredToBed,genePredToFakePsl,genePredToGtf,gff3ToGenePred,gtfToGenePred,pslCDnaFilter,pslMap,pslMapPostChain,pslPosTarget,pslRecalcMatch,pslToBigPsl,transMapPslToGenePred,wigToBigWig} \
  && chmod a+x /binaries/*
...
# also modify the Toil default resource rules in order to allow this container to be used for unit tests
RUN sed -i "s/maxDisk = self.physicalDisk/pass/g" /usr/local/lib/python3.8/dist-packages/toil/batchSystems/singleMachine.py && \
    sed -i "s/maxCores = self.numCores/maxCores = 8/g" /usr/local/lib/python3.8/dist-packages/toil/batchSystems/singleMachine.py && \
    sed -i "s/maxMemory = self.physicalMemory/pass/g" /usr/local/lib/python3.8/dist-packages/toil/batchSystems/singleMachine.py
