...
WORKDIR /hal
# 2020-07-08 snapshot
ADD https://github.com/ComparativeGenomicsToolkit/hal/archive/f8f3fa2dada4751b642f0089b2bf30769967e68a107
...
COPY --from=cat-hal /binaries /usr/local/bin.tar.gz hal.tar.gz
RUN tar --strip-components=1 -xzf hal.tar.gz \
  && make -j \
  && mkdir /binaries \
  && mv bin/hal2fasta bin/hal2maf bin/halStats bin/halLiftover /binaries \
